#!/usr/bin/env python3
"""
Скрипт для исправления критических проблем с БД и стратегиями
"""
import os
import sys
...
def fix_bot_manager():
    """Исправляем проблему с сессией в bot/manager.py"""
    ...
    # 1. Исправляем метод _save_signal
    ...
    # 2. Исправляем метод _update_signal_db
...
def fix_telegram_notifications():
    ...
    # Исправляем send_error чтобы не использовать HTML теги
...
def create_improved_strategies():
    """Создаем улучшенные стратегии с защитой от ошибок"""
...
'''
    ...
    with open('src/strategies/safe_multi_indicator.py', 'w') as f:
        f.write(safe_multi_indicator)
    
    print("✅ Создана безопасная версия multi_indicator стратегии")
    
    # 2. Создаем новую консервативную стратегию
...
'''
    ...
    with open('src/strategies/conservative.py', 'w') as f:
        f.write(conservative_strategy)
    
    print("✅ Создана консервативная стратегия")
    
    # 3. Обновляем фабрику стратегий
    factory_update = '''
...
'''
    ...
    with open('src/strategies/factory.py', 'a') as f:
        f.write(factory_update)
    
    print("✅ Обновлена фабрика стратегий")

def update_trading_pairs_strategy():
    """Обновляем стратегию для торговых пар в БД"""
    update_sql = """
-- Обновляем стратегию на безопасную версию
...
"""
    ...
    print("   Выполните: mysql crypto_top_bd_mysql < update_strategies.sql")

def main():
    print("🔧 ИСПРАВЛЕНИЕ КРИТИЧЕСКИХ ПРОБЛЕМ")
    print("=" * 50)
    
    # 1. Исправляем проблему с БД
    fix_bot_manager()
    
    # 2. Исправляем Telegram уведомления
    fix_telegram_notifications()
    
    # 3. Создаем улучшенные стратегии
    create_improved_strategies()
    
    # 4. Обновляем БД
    update_trading_pairs_strategy()
    
    print("\n✅ Все исправления применены!")
    print("\n📋 Следующие шаги:")
    print("1. Выполните SQL скрипт: mysql crypto_top_bd_mysql < update_strategies.sql")
    print("2. Перезапустите бота: systemctl restart crypto-bot")
    print("3. Проверьте логи на отсутствие ошибок")
