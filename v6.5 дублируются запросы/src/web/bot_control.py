...
class BotController:
    ...
    async def _start_real_bot(self) -> bool:
        """Запуск реального бота с проверками"""
        try:
            global bot_manager
            ...
            # Инициализация с реальными данными
            if not await bot_manager.initialize():
                logger.error("❌ Не удалось инициализировать BotManager")
                return False
            
            # Подключение к бирже
            if not await bot_manager.connect_to_exchange():
                logger.error("❌ Не удалось подключиться к бирже")
                return False
            
            # Загрузка торговых пар
            if not await bot_manager.discover_trading_pairs():
                logger.error("❌ Не удалось загрузить торговые пары")
                return False
            
            # Запуск торговли
            success = await bot_manager.start_trading()
            
            if success:
                logger.info("🚀 Торговый бот запущен с реальными данными")
            
            return success
            
        except Exception as e:
            logger.error(f"❌ Ошибка запуска реального бота: {e}")
            return False
