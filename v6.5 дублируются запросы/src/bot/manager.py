...
class BotManager:
    ...
    async def initialize(self):
        """Инициализация с РЕАЛЬНЫМ подключением"""
        try:
            # Подключаем РЕАЛЬНУЮ биржу
            from ..exchange.real_client import get_exchange_client
            self.exchange = await get_exchange_client()
            
            if not self.exchange.is_connected:
                raise Exception("Не удалось подключиться к бирже")
            
            logger.info("✅ Подключен к РЕАЛЬНОЙ бирже Bybit")
