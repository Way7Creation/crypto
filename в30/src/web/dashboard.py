...
let isConnected = false;
let currentStrategies = {}; // Текущие стратегии для каждой пары
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Скрываем выбор стратегии - теперь автоматический
    document.getElementById('strategy-select').style.display = 'none';
    const strategyLabel = document.querySelector('label[for="strategy-select"]');
    if (strategyLabel) {
        strategyLabel.innerHTML = '🧠 Стратегия выбирается автоматически';
    }
});
...
function updateFromStatusData(statusData) {
    ...
    if (statusData.positions_details) {
        updatePositionsInfo(statusData.positions_details);
    }
...
function updateStrategyDisplay() {
    // Найдем или создадим блок для отображения стратегий
    let strategyBlock = document.getElementById('current-strategies');
    if (!strategyBlock) {
        // Создаем новый блок после управления ботом
        const controlCard = document.querySelector('.card');
        strategyBlock = document.createElement('div');
        strategyBlock.className = 'card';
        strategyBlock.id = 'current-strategies-card';
        strategyBlock.innerHTML = `
            <h3>🧠 Автоматический выбор стратегий</h3>
            <div id="current-strategies"></div>
        `;
        controlCard.parentNode.insertBefore(strategyBlock, controlCard.nextSibling);
    }
...
function updateActivePairs(pairs) {
    ...
    checkboxes.forEach(cb => {
        const pairName = cb.parentElement.textContent.trim().split(' ')[0];
        cb.checked = pairs.includes(pairName);
    });
}

// Обновление информации о позициях
function updatePositionsInfo(positions) {
    // Можно добавить отображение открытых позиций
...
function setupEventListeners() {
    ...
    // Сохранение настроек торговых пар
    const savePairsBtn = document.querySelector('.card:nth-child(2) button');
    if (savePairsBtn) {
        savePairsBtn.addEventListener('click', saveTradingPairs);
    }
}
...
function updateConnectionStatus(connected) {
    ...
    if (connected) {
        ...
        text.textContent = 'Соединение: Активно';
    } else {
        statusElement.className = 'status-item error';
        dot.className = 'status-dot error';
        text.textContent = 'Соединение: Потеряно';
    }
}
...
async function startBot() {
    try {
        ...
        const response = await fetch('/api/bot/start', {
            ...
            body: JSON.stringify({ 
                strategy: 'auto'  // Всегда используем автоматический выбор
            })
        });
...
async function saveTradingPairs() {
    try {
        ...
        const pairs = Array.from(checkboxes).map(cb => 
            cb.parentElement.textContent.trim().split(' ')[0]
        );
...
function pauseTrading() {
    showNotification('⏸️ Эта функция в разработке', 'info');
}
...
async function restartBot() {
    if (confirm('Вы уверены, что хотите перезапустить бота?')) {
        try {
            ...
            if (!response.ok) {
                throw new Error('Ошибка перезапуска');
            }
            
            showNotification('🔄 Бот перезапускается...', 'info');
        } catch (error) {
            showNotification('❌ Ошибка: ' + error.message, 'error');
        }
    }
